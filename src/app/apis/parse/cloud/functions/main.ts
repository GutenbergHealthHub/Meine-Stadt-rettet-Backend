...
Parse.Cloud.define('sendEmail', function (request, response) {
  ...
  adapter._sendMail({
    ...
    variables: { firstname: request.params.firstname, lastname: request.params.lastname, link: request.params.link },
    direct: true
  }).then(function () {
...
Parse.Cloud.define('cancelFirstResponder', function (request, response) {
  ...
  emergencyStateQuery.get(request.params.emergencyStateId).then((emergencyState: EmergencyState) => {
    ...
    emergencyState.save().then(() => {
      ...
      sendCancelPushToFirstresponder(installationQuery, emergencyState.emergencyRelation.id, emergencyState.id, {
...
Parse.Cloud.define('cancelEmergency', function (request, response) {
  ...
  EmergencyQuery.get(request.params.emergencyId).then((emergency: Emergency) => {
    emergency.state = 12;
    emergency.save().then((emergency: Emergency) => {
      const installationQuery = new Parse.Query(Parse.Installation);
      ...
      emergencyStateQuery.find().then((emergencyStates: Array<EmergencyState>) => {
...
Parse.Cloud.define('deleteUser', function (request, response) {
  ...
  userService.getById(request.params.userId).then(user => {
    ...
    user.destroy({ useMasterKey: true }).then(user => {
      response.success('User successfully deleted');
    }, (error) => { response.error('Error in deleting user: ' + error.message); });
