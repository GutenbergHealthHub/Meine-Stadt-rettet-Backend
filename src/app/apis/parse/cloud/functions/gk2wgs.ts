...
function pot2wgs(pot) {
    ...
    if (!pot || !pot.x || !pot.y) return null;
    ...
    const a = 6378137.0 - 739.845;
    const fq = 3.35281066e-3 - 1.003748e-5;
    ...
    if (x > 0) l2 = (180 / pi) * Math.atan(y / x);
    if (x < 0 && y > 0) l2 = (180 / pi) * Math.atan(y / x) + 180;
    if (x < 0 && y < 0) l2 = (180 / pi) * Math.atan(y / x) - 180;
